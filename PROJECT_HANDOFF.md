...
- Tidal forces: optional overlay and status panel for the currently selected body, not all bodies at once. It samples total differential gravitational acceleration from all other bodies around that body's surface, chooses the dominant source by `G M / r^3`, and can add a `near-far tide delta a / surface g` trace to the upper band of the third diagnostic plot. The overlay mode can show arrows, tide shape, or both. Tidal arrows use real local `delta a` directions and are scaled against the strongest tidal arrow over the current trajectory, so eccentric orbits visibly dim away from periapsis. Starter presets include close-moon stress, eccentric tidal pulses, and a spin-orbit exchange model. These tidal presets intentionally use orbit input mode, not direct state mode, so editing eccentricity/orbit/phase recomputes the initial state. The spin-orbit exchange model is a first planar constant-time-lag approximation: it evolves selected-body bulk spin, applies equal/opposite angular-momentum exchange to the relative orbit, tracks heat from mechanical energy loss, rotates the body from `spinPhase`, and shows a lagged tidal bulge when active. In `Rotating pair frame`, rendering subtracts the pair-frame angle from `spinPhase`, so a synchronized body should visually stop rotating relative to its companion. Its UI parameters are `k2`, lag, and inertia alpha; the inputs are visible but disabled until the spin-orbit exchange checkbox is active and at least two bodies exist. Roche limit visualization is also wired to the same dominant tidal source: fluid/rigid rings can be drawn around the source, the panel reports both limits and distance ratios, and the third plot can show `distance / fluid Roche` and/or `distance / rigid Roche` with a `1` threshold. Optional Roche breakup accumulates `tidal damage` over an internal dynamical time instead of triggering instantly at the boundary; material choices alter resilience/severity. Breakup is global for active non-star bodies, while the panel and `tidal damage` plot trace report the currently selected body. At damage `>= 1`, the compact body is disabled in the trajectory and rendered as one selectable debris field. Debris modes are `tracer cloud`, `collective mass` back-reaction, or `fragment N-body` when fragment count is at or below user-set `n0`. This remains a teaching model, not a full material-fracture or hydrodynamics solver.
- `TIDAL_FORCES_THEORY_NOTES.md` is the theory/script scaffold for pływy: Love numbers, `Q`, lag models, spin-orbit torque, angular-momentum transfer, and the future professional implementation path.
- Recommended next tidal implementation: calibrate the current planar spin-orbit subsystem and Roche teaching values against a few known scale examples. Track spin angle, spin rate, moment of inertia `I = alpha m R^2`, `k2`, and a dissipation/time-lag parameter; the current code already applies equal-and-opposite angular-momentum exchange so orbital recession/decay follows from the equations. Roche breakup now has tracer, collective-mass, and limited self-gravitating fragment modes; future work would be collision/re-accretion among fragments, better material-strength parameters, and calibration against known Roche-scale examples.
